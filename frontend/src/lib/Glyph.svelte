<script lang="ts">
  ...
  // so even a tall illustration can never overflow the Puck's safe area. Throws on unknown ref.
  import { glyphPath } from "./glyphs";

  let { ref, label = "", size = 24 }: { ref: string; label?: string; size?: number } = $props();
</script>
