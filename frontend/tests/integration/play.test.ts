...
// The play-store loop needs a fully frontend-solvable manifest. The served standard/sharp/expert
// puzzles are now story-first (numeric/compound clues validate.ts does not yet evaluate, plus an
// anchor:true identity board.ts does not yet honour - both the next frontend row). The 4x3 fixture
// (same date 2026-06-29, tier standard) is solvable today, so the store loop is exercised against
// it; the served story bank's shape/load contract is covered by the loader + shapes tests.
const m = JSON.parse(
  readFileSync(resolve(here, "../fixtures/manifest-4x3.json"), "utf8"),
) as PuzzleManifest;
// Easy stays served: an all-direct eq-only grid whose anchor is the subject (cats[0]), which the
// current frontend resolves + solves. Same calendar date -> the composite-day streak stays 1.
