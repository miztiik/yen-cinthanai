...
- Landing: PLAY (one tap; first-ever = Easy cold-open, else resumes the last-played tier), gear, flame+best, and a colour-coded difficulty pill (ascending TierMeter bars + the tier NAME in its standardized colour) bound directly under PLAY as one CTA cluster; tapping it opens the difficulty sheet. No separate "more puzzles" drawer, no tier picker screen, no tutorial. The single-line "Yen Cinthanai" wordmark is a container-query lockup (fits one line phone-to-desktop) in thin uppercase Josefin Sans with the cross-app rainbow gradient.
